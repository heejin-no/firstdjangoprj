...
def post_new(request):
    # request는 사용자의 요펑, method는 요청방식
    # GET방식으로 들어온 요청인지 POST방식으로 들어온 요청인지 검사.
    # form의 method="post"이므로 form을 사용해 들어온 자료는 무조건 post여야함
    if request.method == "POST":
        form = PostForm(request.POST)
        # .is_valid()는 form을 통해 넘어온 자료가 우효한 자료인지 검증
        if form.is_valid():
            # 우선 form 에서 입력한 title,text 정보를 넘겨받고
            ...
            # 6개의 컬럼에 대한 정보를 완전히 입력 후 DB반영
            post.save()
            # 글 다썻으면 detail 페이지에서 쓴글 확인.
...
def post_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        # 민약 post방식이 아닌 경우라면 get방식 이므로 수정 직전임
        # 따라서 폼으로 다시 연결해야함. 이 때의 폼은 수전용 폼이며
        # 수정용 폼에는 기존에 써놨었던 글이 먼저 입력되어 있어야하므로
